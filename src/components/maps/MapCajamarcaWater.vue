      ...
      d3.json('/geojson/peru_provincial_simple.geojson').then((geoData) => {
        ...
        svg
          ...
          .attr('fill', (d) => {
            ...
            return provinceData ? getCoverageColor(provinceData.percentage_with_water) : 'rgba(255, 255, 255, 1)'
          })
          .attr('stroke', 'black')
          .attr('stroke-width', 1)
        ...
        svg
          ...
          .each(function(d) {
            const provinceData = props.provincesData.find(p => normalizeString(p.name) === normalizeString(d.properties.NOMBPROV))
            const provinceName = d.properties.NOMBPROV
            const waterPercentage = provinceData ? `(${provinceData.percentage_with_water.toFixed(2)}%)` : ''
